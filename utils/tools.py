...
def compute_metrics(actual: np.ndarray, predicted: np.ndarray) -> dict:
    """
    Compute the five evaluation metrics on denormalized kW values.

    Both arrays are flattened to 1-D before computation so inputs of shape
    (n_samples,) or (n_samples, n_horizons) are handled identically.

    Parameters
    ----------
    actual    : array-like of ground-truth values in kW
    predicted : array-like of predicted values in kW

    Returns
    -------
    dict with keys 'mae', 'rmse', 'r2', 'mbe', 'smape'
        smape is expressed as a percentage in the range [0, 200].
        Samples where BOTH actual AND predicted are 0 kW are excluded
        from the sMAPE computation (requirement 6.7).
    """
    ...
    if a.shape != p.shape:
        raise ValueError(
            f"actual and predicted must have the same number of elements; "
    ...
    # sMAPE: exclude samples where BOTH actual and predicted are 0
    both_zero = (a == 0.0) & (p == 0.0)
    denom = np.abs(a) + np.abs(p)
    # Guard remaining zero denominators (one is 0 but not both) — treated as
    # 200 % contribution per the sMAPE definition when denom == 0 is impossible
    # after excluding both-zero pairs, but kept safe with clip.
    denom_safe = np.where(both_zero, 1.0, np.maximum(denom, 1e-12))
    smape_values = 200.0 * np.abs(a - p) / denom_safe
    smape_values = np.where(both_zero, 0.0, smape_values)   # zero-out excluded samples
    n_valid = np.sum(~both_zero)
    smape = float(np.sum(smape_values) / n_valid) if n_valid > 0 else 0.0
